...
fixed4 ComputeFinalColor(float2 uvReflection, float2 uvTex) 
{
	...
	//uvReflection.x = (uvReflection.x - 0.4) / 2.0 + 0.4;
	//uvReflection.y = (uvReflection.y - 0.4) / 2.0 + 0.4;
	//uvTex.x = ((uvTex.x - 0.5) / 2.0) + 0.5;
	//uvTex.y = ((uvTex.y - 0.5) / 2.0) + 0.5;
...
{
	//return isEyeLeft() ? fixed4(1,0,0,1) : fixed4(0,1,0,1);
	//uvReflection.x = ((uvReflection.x - 0.5) / 2.0) + 0.5;
	//uvReflection.y = ((uvReflection.y - 0.5) / 2.0) + 0.5;
